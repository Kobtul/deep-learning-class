...
class Network:
    ...
    def construct(self, args):
        with self.session.graph.as_default():
            ...
            # activations are none, relu, tanh and sigmoid.c
            ...
            for i in range(layers) :
                hidden_layer = tf.layers.dense(hidden_layer, args.hidden_layer, activation=aktiv ,name="hidden_layer"+str(i))
    ...
    def evaluate(self, dataset, images, labels):
        temp = self.session.run(self.summaries[dataset], {self.images: images, self.labels: labels})

    def get_accuracy(self, datase, images, labels):
...
if __name__ == "__main__":
    ...
    print("{:.2f}".format(network.get_accuracy("test", mnist.test.images, mnist.test.labels)*100))